...
// Style function for OpenHistoricalMap vector tiles
function createOHMStyle() {
  return new Style({
    fill: new Fill({ color: 'rgba(139, 92, 246, 0.1)' }), // Light purple fill
    stroke: new Stroke({
      color: 'rgba(139, 92, 246, 0.7)', // Purple stroke
      width: 1.5,
    }),
  });
}
...
function MapContent(): JSX.Element {
  ...
  const [overlays, setOverlays] = useState<HistoricalOverlay[]>(DEFAULT_OVERLAYS);
  const [overlayLoadingState, setOverlayLoadingState] = useState<Record<string, boolean>>({});
  ...
  // Create a layer for an overlay based on its source type
    ...
    async (overlay: HistoricalOverlay): Promise<BaseLayer | null> => {
      ...
      try {
        switch (overlay.source) {
          case 'allmaps': {
            if (!overlay.annotationUrl) {
              console.warn(`Allmaps overlay ${overlay.id} has no annotation URL`);
              return null;
            }

            // Dynamically import Allmaps to avoid SSR issues
            const { WarpedMapLayer, WarpedMapSource } = await import('@allmaps/openlayers');

            const warpedMapSource = new WarpedMapSource();
            await warpedMapSource.addGeoreferenceAnnotation(overlay.annotationUrl);
          ...
          case 'ohm': {
            const ohmLayer = new VectorTileLayer({
              source: new VectorTileSource({
                format: new MVT(),
                url: overlay.tileUrl || 'https://vtiles.openhistoricalmap.org/maps/osm/{z}/{x}/{y}.pbf',
          ...
          case 'custom': {
            if (!overlay.tileUrl) {
              console.warn(`Custom overlay ${overlay.id} has no tile URL`);
              return null;
            }

            // Assume XYZ tiles for custom sources
          ...
          default:
            console.warn(`Unknown overlay source: ${overlay.source}`);
            return null;
        }
      } catch (error) {
        console.error(`Failed to create layer for overlay ${overlay.id}:`, error);
        return null;
      } finally {
  ...
  // Handle toggling an overlay on/off
  const handleToggleOverlay = useCallback(
    async (id: string) => {
      ...
      if (overlay.enabled) {
        // Currently enabled, so we're disabling - remove layer
        if (existingLayer) {
          map.removeLayer(existingLayer);
          overlayLayersRef.current.delete(id);
        }
      } else {
        // Currently disabled, so we're enabling - add layer
        if (existingLayer) {
          ...
          if (newLayer) {
            ...
            // Insert below the events layer
            const layers = map.getLayers().getArray();
            const eventsLayerIndex = layers.findIndex((l) => l.get('layerId') === 'events');
  ...
  // Handle reordering overlays
  const handleReorderOverlays = useCallback((reorderedOverlays: HistoricalOverlay[]) => {
    setOverlays(reorderedOverlays);

    // Reorder layers on the map based on zIndex
    const map = mapRef.current;
    if (!map) return;

    // Get all overlay layers and reorder them
    ...
    reorderedOverlays.forEach((overlay) => {
      ...
      if (layer && overlay.enabled) {
        ...
        layers.remove(layer);
        // Find position before events layer
        const eventsLayerIndex = layers.getArray().findIndex((l) => l.get('layerId') === 'events');
        if (eventsLayerIndex > 0) {
  ...
  // Calculate min/max years from all overlays
  const minYear = Math.min(...overlays.map((o) => o.yearRange[0]));
  const maxYear = Math.max(...overlays.map((o) => o.yearRange[1]));

  // Filter overlays based on timeline selection
  const filteredOverlays = isTimelineEnabled
    ? overlays.filter((o) => selectedYear >= o.yearRange[0] && selectedYear <= o.yearRange[1])
    : overlays;
